...
samples with negative label), *false positives* (FP, samples with a predicted positive label but true negative label),
...
:math:`\text{prec}_a`, and :math:`\text{neg}_a` in the following for sensitivity, specificity, precision, and
...
:math:`0.025` for the label *female* and :math:`0.05` for the label *male*. While the absolute difference between
sensitivity scores is only :math:`0.025`, the classifier still achieves twice the sensitivity for the label *male*.
...
sufficiency focuses on error parity among individuals who are given the same decision :cite:p:`fairness-Steinberg2020`.
...
*Note*: Since the goal is to quantify the fairness of the underlying AI model itself, the following metrics for
...
computation of *specificity* and *negative predictive value* requires the presence of TN.
